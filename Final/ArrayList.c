    ...
    if(this != NULL && pFunc != NULL && (order == 0 || order == 1)) {
        ...
        for(int i = 0; i < tam; i++) {
            for(int j = i + 1; j < tam; j++) {
                void** pElementI = this->pElements[i];
                void** pElementJ = this->pElements[j];
                int res = pFunc(pElementI, pElementJ);
                if((res == 1 && order == 1) || (res == -1 && order == 0)) {
                    this->pElements[j] = pElementI;
                    this->pElements[i] = pElementJ;
                }
            }
        }
        returnAux = 0;
...
ArrayList* al_filter_localidades(ArrayList* this, int (*pFunc)(void*, char*)) {
    ...
    if(this != NULL && pFunc != NULL) {
        ...
        for(int i = 0; i < al_len(this); i++) {
            pFunc(this->pElements[i], this->pElements[i]);
        }
